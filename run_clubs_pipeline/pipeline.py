...
try:
    ...
    from run_clubs_pipeline.website_worker import (
        run_website_checks,
        website_results_by_club,
        write_website_results,
    )
except ModuleNotFoundError:  # pragma: no cover - direct script execution fallback
    from manual_social_queue import (
        due_social_review_tasks,
        ensure_social_capture_template,
        next_due_at,
        write_social_queue,
...
def _seed_registry(path: Path) -> None:
    ...
    starter = [
        ...
        ClubSource(
            club_id="sample_hfx_instagram",
            club_name="Sample Harbour Runners",
        ...
        ClubSource(
            club_id="sample_hfx_facebook",
            club_name="Sample Dartmouth Run Group",
...
def _update_registry_timestamps(sources: List[ClubSource], website_by_club: dict[str, object], now_iso: str) -> List[ClubSource]:
    ...
    for source in sources:
        ...
        if source.source_type in {"instagram", "facebook"}:
            if not source.manual_review_due_at:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run clubs pipeline: website checks + manual social queue."
    )
    parser.add_argument(
        "--seed",
        action="store_true",
        help="Seed a starter source registry if empty.",
    )
